    ...
    /* componentDidMount() {
        ...
        $('.grid-stack').each(function () {
            ...
            _.each(items, function (node) {
                grid.addWidget($('<div><div class="grid-stack-item-content" ><button type="button" class="close" d' +
                        'ata-dismiss="modal" aria-label="Close" ><span aria-hidden="true">&times;</span><' +
                        '/button></div></div>'), node.x, node.y, node.width, node.height);
            }, this);
        });
        ...
        $('.container-fluid').on("click", ".grid-stack-item-content", function () {
            ...
            var controlIcons = [
                {
                    name: 'information',
                    code: '<span class="iconfont icon-infomation" />'
                }, {
                    name: 'title',
                    code: 'icon-title'
                }, {
                    ...
                    name: 'line-char',
                    code: 'icon-line-chart'
                }, {
                    name: 'bar-chart',
                    code: 'icon-bar-chart'
                }, {
                    name: 'split',
                    code: '<span class="split" />'
                }, {
                    name: 'state',
                    code: '<span class="state">' + stateContentExample + '</span>'
                }
            ];
            if ($parent.offset() != null) {
                ...
                if ($editor.length == 0) {
                    $('.container-fluid').append('<div class="grid-stack-control" style="display: block; left: ' + _left + 'px; top: ' + _top + 'px;"><div class="grid-stack-control-title"><span>yourTitle<span></div><div class' +
                            '="grid-stack-control-bar">' + controlIcons[0].code + controlIcons[0].code + controlIcons[0].code + controlIcons[6].code + controlIcons[7].code + controlIcons[6].code + '<span class="toll"><span class="iconfont ' + controlIcons[1].code + ' selected" /><span class="iconfont ' + controlIcons[2].code + '" /><span class="iconfont ' + controlIcons[3].code + ' select select-hide" /><span class="iconfont ' + controlIcons[4].code + ' select select-hide" /><span class="iconfont ' + controlIcons[5].code + ' select select-hide" /></span></div></div>');
                } else if ($editor.css('display') == 'block' && $editor.css('left') == (_left + 'px') && $editor.css('top') == (_top + 'px')) {
